...
外の専用ディレクトリで作って破棄する（`keep_intermediates: true` で `output/tmp` へ写す）。
ワークスペースに置くのは完成した MP4 だけなので、描画中にそこのファイルを変えても ffmpeg を
操れない。
...
- `audio`（必須）— ワークスペース相対のナレーション音声。その長さがページの表示時間。WAV・MP3・M4A/MP4・FLAC・
  Ogg/Opus・AAC・WebM/MKA・AIFF のどれか。中身がプレイリストや連結リストのファイルは拒否する。
