...
外部を読み書きすることを防ぐ。ただし root は**自身のパス**は通常どおり解決する
ため、ベースディレクトリはそこに張った root では守られない。`makeWorkspaceDir`
は `work_dir` 上の `os.Root` 経由で `<id>` を作成し、ベースの実パスを
`<実 work_dir>/<id>` と比較して、リンクだったワークスペースを拒否する。この比較が
要であり、理由はこのパスが ffmpeg に渡され、ffmpeg はいかなる root の外でも解決
するため。ffmpeg は `os.Root` を継承できないため、入力は spawn 直前に Lstat で
再検証し、出力パスは root 経由で消してリンクが辿られないようにする。
検証〜spawn 間の競合はローカル単一ユーザーの脅威モデル下で受容する。隙間はそれだけでは
ない: レンダリング中にワークスペースのディレクトリをリンクへすり替えること、ffmpeg が
ファイルの中身から形式を選ぶことは ADR-0009 に記録し、ここでは閉じていない。
