...
1ワークスペース = 1デッキ（`<root>/<id>/`）。ルートはサーバー既定
（`~/.video-studio`）か、`workspace_root` で渡す**エージェント用意**の絶対パス。
エージェント用意ルートはエージェント書込可能なので、サーバー側の全ファイル操作は
...
- **インプロセスツールテスト** — `mcpserver.Server.Call` で名前指定してツールを
  ...
  （workspace_root・シンボリックリンク拒否含む）を通す。
