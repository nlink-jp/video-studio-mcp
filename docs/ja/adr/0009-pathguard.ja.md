...
- **直した（原因ごと）:** ffmpeg が書いて読み戻すもの —— 字幕画像、セグメント、連結リスト、章、字幕、描画中の完成品 —— は、
  ワークスペースの外に描画ごとに作る推測できない名前の専用ディレクトリ（0700）で扱う。完成品だけを、ワークスペースの
  `os.Root` を通して一時名で作ってから名前を替えて置くので、そこに置かれたリンクはたどらずに置き換え、外へ通じる
  構成要素は拒む。`keep_intermediates` は中間物を同じ方法で `output/tmp` へ写す（成功でも失敗でも）。
- **入力の形式を固める:** ワークスペースから読むのはページの入力だけで、画像は `image2`・パターンなし（`%` も名前）、
  デコーダはファイル先頭のバイトで決める（拡張子で決めると、`.png` という名前の JPEG で ffmpeg が終わらなくなった）。
...
- **受け入れた残余**（運用者の方針「完璧より総合的なリスク評価」）: ページの入力を判定してから ffmpeg が開くまでの間に
  ...
  解釈できる画像・音声だけで、資格情報のファイルは当たらない。壊れた画像で ffmpeg が終わらない件（この変更の前から）も
  残る。pathguard は v0.3.0 に上げた（このサーバーでの挙動は変わらない）。
