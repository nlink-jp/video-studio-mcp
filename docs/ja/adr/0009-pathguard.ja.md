...
- **直した（原因ごと）:** ffmpeg が書いて読み戻すもの —— 字幕画像、セグメント、連結リスト、章、字幕、描画中の完成品 —— は、
  ...
  サンドボックス内の書き手からの上書きを実測）。キャッシュディレクトリはどのレーンでも書けない（読み取りレーンは
...
- **入力の形式を固める:** ワークスペースから読むのはページの入力だけで、画像は `image2`・パターンなし（`%` も名前）。
  ...
  PNG、画像でないバイト —— と 8192×8192 画素より大きいもの（小さな PNG が巨大な画面を名乗れ、デコードはサーバー自身の
  プロセスで走る）は、ffmpeg を動かす前に `invalid_manifest` で拒む。ffmpeg に読めない画像を渡すと、ループする
...
- **受け入れた残余**（運用者の方針「完璧より総合的なリスク評価」）: ページの入力を判定してから ffmpeg が開くまでの間に
  ...
  ffmpeg は終わらないが、止まるのはその呼び出し側自身の描画だけである。キャッシュディレクトリも、どのサンドボックスにも
  入っていない同じユーザーのプロセス（利用者自身のシェルなど）には書ける —— それは利用者本人ができることの範囲である。
