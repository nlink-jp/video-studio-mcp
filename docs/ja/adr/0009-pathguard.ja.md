...
## Amendment (2026-09-22, v0.6.2): ffmpeg の入力の形式を固める — 残りは受け入れる
...
- **実測:** 中身が ffconcat のページ（拡張子は `.wav`、`file 'k'`、k はワークスペース内のリンクで行き先は外）を ffmpeg は
  たどり、外のファイルの音声が出力に入った。絶対パスを書いた ffconcat は ffmpeg の safe の既定が拒み、`.wav` という名前の
  HLS は ffmpeg が HLS として扱わなかった。
- **直した:** 画像と字幕画像は `image2`・パターンなし（`%` も名前として読む）、音声と ffprobe は音声の形式だけの許可一覧
  （wav・mp3・mov/mp4/m4a・flac・ogg・aac・matroska/webm・aiff。連結リストもプレイリストも入れない）、読める経路は
  `file` だけ。受け付けている形式（PNG/JPG × wav・mp3・m4a・flac・aiff・opus・webm・mka・aac）がそのまま通り、細工した
  ファイルが「形式が一覧にない」で拒まれることを、サーバーと同じ引数の並びで確かめた。連結リストには制御文字を含むパスを
  書かない（改行が項目を終わらせ、`-safe 0` のリストに自前の項目を足せる）。
- **受け入れた残余**（運用者の方針「完璧より総合的なリスク評価」）: レンダリング中にワークスペースのディレクトリ自体を
  リンクへすり替える競合、サーバーが書いたリスト（連結・章・字幕）の起動直前の再検証、書き込み先の判定（`WriteFileAtomic`
  はワークスペース内に仕込まれたディレクトリのリンクをたどる。出力名 `.env` は `.env.mp4` になる）。どれも、時間合わせか
  ワークスペース内にリンクを仕込むことが要り、書かれるのはサーバーが決めた名前と中身だけ。
